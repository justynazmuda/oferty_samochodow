...
samochody_new_rules %>%
  ...
  tbl_summary(
    ...
    label = price_in_pln ~ "Cena") %>%
  modify_header(label ~ "**Zmienna**") %>%
  modify_caption("**Tabela 1. Rozkłąd cen według marki. **") %>%
  bold_labels() %>% 
  add_p(pvalue_fun = ~ style_pvalue(.x, digits = 2)) %>%
as_gt() %>%
tab_style(
...
H0:  Średnie (lub mediany) cen samochodów są takie same dla wszystkich marek. Marka nie ma wpływu na cenę auta.
H1: Przynajmniej jedna marka ma inną medianę ceny niż pozostałe. Marka wpływa na cenę auta.
α = 0,05
```{r}
...
ggplot(samochody_new_rules, aes(x = reorder(brand, price_in_pln, median), y = price_in_pln)) +
  ...
  xlab("Marka") + ylab("Cena w PLN")
```
## 2. Hipoteza 2 -> Rok wyprodukwoania auta nie ma wpływu na cenę. 
H0:  Średnie (lub mediany) cen samochodów są takie same dla wszystkich lat wyprodukowania. Rok wyprodukowania nie ma wpływu na cenę auta.
H1: Przynajmniej jeden rok wyprodukowania ma inną medianę ceny niż pozostałe. Rok wyprodukowania wpływa na cenę auta.
α = 0,05
...
ggplot(samochody_new_rules, aes(x = reorder(year, price_in_pln, median), y = price_in_pln)) +
  geom_boxplot() +
  coord_flip() +
  ggtitle("Rozkład cen samochodów dla różnych lat ich wyprodukowania") +
  xlab("Rok wyprodukowania") + ylab("Cena w PLN")
```
## 3. Hipoteza 3 -> Przebieg auta nie ma wpływu na cenę. 
H0:  Średnie (lub mediany) cen samochodów są takie same dla wszystkich ilości przebiegu danego auta. Przebieg nie ma wpływu na cenę auta.
H1: Przynajmniej jeden przebieg ma inną medianę ceny niż pozostałe. Przebieg wpływa na cenę auta.
α = 0,05
...
ggplot(samochody_new_rules, aes(x = reorder(mileage, price_in_pln, median), y = price_in_pln)) +
  geom_boxplot() +
  coord_flip() +
  ggtitle("Rozkład cen samochodów dla różnych przebiegów") +
  xlab("Przebieg") + ylab("Cena w PLN")
```
